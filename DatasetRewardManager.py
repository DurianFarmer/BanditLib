...
class DatasetRewardManager():
    ...
    def runAlgorithms(self, algorithms, diffLists):
        ...
        filenameWriteRegret = os.path.join(
            self.save_address, 'AccRegret' + timeRun + '.csv')
        ...
        with open(fileName, 'r') as f:
            ...
            for i, line in enumerate(f, 1):
                ...
                for article in pool_articles:
                    ...
                    articlePool.append(
                        Article(article_id, FeatureVectors[article_id]))
                ...
                if i % 100 == 0:
                    ...
                    if i % 1000 == 0:
                        self.batchRecord(algorithms, i, tstart,
                                         RandomChoice, AlgPicked)
                        self.write_regret_to_file(
                            filenameWriteRegret, algorithms, BatchCumlateRegret, i, RandomChoice.regret)
            if self.plot:
                self.plot_result(algorithms, BatchCumlateRegret,
                                 tim_, RandomChoiceRegret, AlgRewardRatio_vsRandom)
