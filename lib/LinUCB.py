...
class LinUCBUserStruct:
    ...
    def updateParameters(self, articlePicked_FeatureVector, click):
        ...
        self.A += np.outer(articlePicked_FeatureVector,
                           articlePicked_FeatureVector)
...
class Hybrid_LinUCB_singleUserStruct(LinUCBUserStruct):
    ...
    def updateParameters(self, articlePicked_FeatureVector, click):
        ...
        LinUCBUserStruct.updateParameters(
            self, articlePicked_FeatureVector, click)
...
class Hybrid_LinUCBUserStruct:
    def __init__(self, featureDimension,  lambda_, userFeatureList):
        ...
        for i in range(len(userFeatureList)):
            ...
            self.users.append(Hybrid_LinUCB_singleUserStruct(
                userFeatureList[i], lambda_, i))
    ...
    def updateParameters(self, articlePicked_FeatureVector, click, userID):
        ...
        temp = np.dot(np.transpose(
            self.users[userID].B), self.users[userID].AInv)
        ...
        self.A_z = self.A_z + np.outer(z, z) - \
            np.dot(temp, self.users[userID].B)
    ...
    def getProb(self, alpha, article_FeatureVector, userID):
        ...
        temp = np.dot(np.dot(np.dot(self.A_zInv, np.transpose(
            self.users[userID].B)), self.users[userID].AInv), x)
        mean = np.dot(self.users[userID].UserTheta,  x) + np.dot(self.beta, z)
        s_t = np.dot(np.dot(z, self.A_zInv),  z) + \
            np.dot(np.dot(x, self.users[userID].AInv),  x)
        -2 * np.dot(z, temp) + np.dot(np.dot(np.dot(x,
                                                    self.users[userID].AInv),  self.users[userID].B), temp)
...
class Hybrid_LinUCBAlgorithm(object):
    def __init__(self, dimension, alpha, lambda_, userFeatureList):
        self.dimension = dimension
        self.alpha = alpha
        self.USER = Hybrid_LinUCBUserStruct(
            dimension, lambda_, userFeatureList)
    ...
    def updateParameters(self, articlePicked, click, userID):
        self.USER.updateParameters(
            articlePicked.contextFeatureVector, click, userID)
